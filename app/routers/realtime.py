...
[웹 검색 도구 사용 기준]
web_search 함수는 아래 상황에서만 사용하세요:
- 사용자가 최신 정보나 사실 확인을 요청할 때: 뉴스, 날씨, 주가, 스포츠 결과 등
- 사용자가 특정 정보를 물어봤는데 당신이 확실히 알 수 없을 때
- "검색해줘", "찾아봐", "알아봐줘" 등 명시적으로 검색을 요청할 때

다음 상황에서는 절대 사용하지 마세요:
- 일반적인 일기 대화 (검색 없이 충분히 공감하고 대화 가능한 상황)
- 이미 알고 있는 일반 상식 질문
- 감정 공감이나 일상 이야기"""
...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "인터넷에서 최신 정보를 검색합니다. "
            "사용자가 뉴스·날씨·주가·스포츠 등 최신 정보를 요청하거나, "
            "'검색해줘', '찾아봐', '알아봐줘' 등 명시적으로 검색을 요청할 때만 호출하세요. "
            "일반 일기 대화, 감정 공감, 이미 알고 있는 상식 질문에는 사용하지 마세요. "
            "반환값: 검색 결과 요약 및 출처."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "검색할 쿼리. 한국어 또는 영어로 구체적으로 작성.",
                },
            },
            "required": ["query"],
        },
...
class SessionConfig(BaseModel):
    # model은 서버 설정(OPENAI_REALTIME_MODEL)이 결정 — 하위호환으로 받기만 하고 쓰지 않음
    model: str | None = None
...
async def get_token(body: TokenRequest, current_user: dict = Depends(get_current_user)):
    ...
    # 모델은 서버 설정이 단일 소스 — 클라이언트가 보낸 값은 참고만 하고 신뢰하지 않음
    model = settings.openai_realtime_model
    url = f"{settings.openai_base_url}/realtime/client_secrets"

    # GA 스키마: voice/transcription/turn_detection은 audio.input/output에 nested.
    # flat 구조(구 beta)로 보내면 400 떨어짐.
    session_payload = {
        "session": {
            "type": "realtime",
            "model": model,
    ...
    logger.info(f"🌐 OpenAI Realtime — POST {url} (model={model})")
    ...
    if not token_value:
        logger.error(f"❌ OpenAI 응답에 토큰 없음: {data}")
        raise HTTPException(status_code=502, detail="OpenAI 응답에 ephemeral token이 없습니다.")
    session_obj = data.get("session") or {}
    session_id = session_obj.get("id") or str(uuid.uuid4())
    expires_at = data.get("expires_at")
    webrtc_url = f"{settings.openai_base_url}/realtime/calls"
    model_label = session_obj.get("model") or model
